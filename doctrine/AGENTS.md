...
### Index of Daemon Doctrine

Each daemon's full operating doctrine now lives beside its code, not inline
here. Open the relevant file when a task touches that daemon's behavior,
write surfaces, output contract, or test discipline. Paths are relative to
the Abelard monorepo root (`/mnt/c/Users/mdiba/Code/Abelard/`).

- **News Watch Daemon** — `daemons/news_watch_daemon/AGENTS.md`. Open when
  reading Briefs, changing themes or synthesis, or asked "what's changed."
- **Research Daemon** — `daemons/research_daemon/AGENTS.md`. Open when
  pulling quotes, filings, insider/13F data, or running watchlist sweeps.
- **Biz Daemon** — `daemons/biz_daemon/AGENTS.md` (STUB — pending Mando
  review). Open when reading 4chan /biz/ /smg/ retail-sentiment snapshots.
- **Chatter Daemon** — `daemons/chatter_daemon/AGENTS.md` (STUB — pending
  Mando review). Open when reading multi-source retail-chatter scans.
- **abelard_common** — `daemons/common/AGENTS.md` (STUB — pending Mando
  review). Open when touching shared daemon primitives (DaemonError, the
  ticker-noise filter, 4chan fetch, HTTP client).
