...
The L2 confirmation pass for consensus (Detector A) is a **calendar item** (~mid-Sep 2026), passive; the collector is already archiving the data. The corrected gate-attribution (docs/m0c_report.md §4b) found: the 2026 roster is **healthy** (not collapsed — that was a diagnostic bug), censoring is ruled out, and the real failure is **dispersal** — skilled wallets no longer co-participate on the same markets (≥3 on one 2026 market: never; vs 22 convergences in 2025-H1). Dispersal is visible pre-split, so it leans toward the architect's mechanism (c) real-and-persistent — but is confounded by the network-truncated + horizon-biased 2026 sample. **Net (v1.8 §3): September is downgraded to "run it, expect a formality"; the cross-venue spread becomes the primary forward hope.** Either way, nothing is built for Detector A now — this milestone (Detector B) is where the near-term build goes.
