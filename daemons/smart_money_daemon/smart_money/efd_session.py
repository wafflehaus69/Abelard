...
def bootstrap(user_agent: str, probe: bool = True) -> requests.Session:
    ...
    The data endpoint is reachable via plain requests — post_data (agreement session +
    X-CSRFToken) returns 200 for report_type 11 (PTR) and 7 (annual). Measured, not
    assumed: 36 probes / 13 days, weekday n=22, 100% at every sampled hour
    (recon/EFD_WAF_FINDING.md — DATED, expires 2026-11-06). The one real constraint is
    rate shaping under unpaced bursts, so pace requests."""
    s = requests.Session()
